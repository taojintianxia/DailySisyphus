...
public class SpringStepsFactory extends AbstractStepsFactory {
    ...
    private ThreadLocal<HashMap<Class<?>, Object>> stepCache = new ThreadLocal<>();

    private HashMap<Class<? extends BaseStep>, BaseStep> systemSteps =
            new HashMap<>();

    public SpringStepsFactory(Configuration configuration, ApplicationContext context,
            Collection<BaseStep> systemSteps) {
        super(configuration);
    ...
    protected boolean isAllowed(Class<?> type) {
        ...
        return type != null && !Modifier.isAbstract(type.getModifiers()) && isSteps;
    }
