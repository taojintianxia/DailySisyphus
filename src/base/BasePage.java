...
public abstract class BasePage {
	...
	protected final ODWebDriverProvider driverProvider = ODWebDriverProvider.getInstance();
	...
	/**
	 * @param give
	 *            the locator in format of "id=","css=","name=","xpath=",etc...
	...
	protected boolean isTextPresentInElement(By elementLocator, String text) {
		...
		return wait.until(ExpectedConditions.textToBePresentInElement(elementLocator, text));
	}
	...
	private void nonBlockingClick(WebElement element) {
		new Actions(webDriver).click(element).perform();
		// ((JavascriptExecutor)
		// webDriver).executeScript("arguments[0].click()", element);
	}

	protected void uploadFile(WebElement loadFileWidget, String absolutePath) throws FileNotFoundException {
		if (StringUtils.isBlank(absolutePath)) {
			throw new IllegalArgumentException("absolute path is blank");
		} else {
			if (!new File(absolutePath).exists()) {
				throw new FileNotFoundException("expeteced upload file does not exist");
			}
		}

		nonBlockingClick(loadFileWidget);
		loadFileWidget.sendKeys(absolutePath);
	}
