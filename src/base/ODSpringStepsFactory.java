...
public class ODSpringStepsFactory extends AbstractStepsFactory {
	...
	public ODSpringStepsFactory(Configuration configuration, ApplicationContext context, Collection<BaseStep> systemSteps) {
		super(configuration);
	...
	public Object createInstanceOfType(Class<?> type) {
		if (systemSteps.get(type) != null)
			return systemSteps.get(type);
		...
		for (String name : context.getBeanDefinitionNames()) {
			...
			if (type.equals(beanType)) {
				...
				if (stepCache.get() == null)
					stepCache.set(new HashMap<Class<?>, Object>());
				if (null == stepCache.get().get(type))
					stepCache.get().put(type, o);
				return o;
			}
		}
